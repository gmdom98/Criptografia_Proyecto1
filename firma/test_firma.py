...
def test(sig_ver,ver_ver):
    ...
    sig_ver.append([rsa_temp,ecdsa521_temp,ecdsa571_temp])
    ver_ver.append([rsa_pss_ver,ecdsa_521_ver,ecdsa_571k_ver])
            
    return sig_ver,ver_ver
...
def average():
    ...
    fig.add_subplot(941)
    plt.plot(num,sign_times[0],'-b',label='rsa')
    #plt.plot(num,auxrsa,'-r',label='rsa')
    plt.plot(num,sign_times[1],'-g',label='ecdsa521')
    plt.plot(num,sign_times[2],'-r',label='ecdsa571')
